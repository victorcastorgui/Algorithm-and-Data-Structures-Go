...
func genericTransposeArr[T any](pixels [][]T) [][]T {
	for i := 0; i < len(pixels); i++ {
		for j := i; j < len(pixels[0]); j++ {
...
func genericReverseArr[T any](pixels [][]T) [][]T {
	for i := range pixels {
		leftSide := 0
		rightSide := len(pixels[i]) - 1
		for leftSide < rightSide {
			pixels[i][leftSide], pixels[i][rightSide] = pixels[i][rightSide], pixels[i][leftSide]
			leftSide++
			rightSide--
		}
	}
...
func RotateImage(pixels [][]int) [][]int {
	...
	genericTransposeArr(pixels)
	genericReverseArr(pixels)
